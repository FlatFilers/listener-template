...
export function matchSheets(
  ...
  sheetConfigs.forEach((config, configIndex) => {
    ...
    let matchedSheet = existingSheets.find((sheet) => sheet.name === config.name && !usedSheets.has(sheet.id))

    // If no name match, try to match by slug if available
    if (!matchedSheet && config.slug) {
      matchedSheet = existingSheets.find((sheet) => sheet.slug === config.slug && !usedSheets.has(sheet.id))
    }
